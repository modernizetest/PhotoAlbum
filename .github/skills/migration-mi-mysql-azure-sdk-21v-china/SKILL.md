...
Your job is to migrate from MySQL to Azure Database for MySQL with Azure SDK and managed identity in the Mooncake cloud for secure, credential-free authentication.
...
## Upgrade code to use Managed Identity in Azure MySQL (China Cloud)

Migrate Azure MySQL authentication to managed identity approach using AzureMysqlAuthenticationPlugin.
...
## Add dependency for Managed Identity in Azure MySQL (China Cloud)
...
## Add properties for Managed Identity in Azure MySQL (China Cloud)

Add Azure managed identity name to the configuration.
...
3. Add the property for configuring access credentials to China Cloud in Java code or JDBC URL property, requiring configuration in only one place.
   Add in java code:
   ```diff
    + properties.put("azure.authorityHost",AzureAuthorityHosts.AZURE_CHINA);
   ```
   Add in JDBC URL property:
   ```diff
    + url={MYSQL_URL}&azure.authorityHost=https://login.partner.microsoftonline.cn
   ```