...
- **Rule 1 — Single task only**: Always create a **single** upgrade task that encompasses all necessary changes (SDK-style conversion, TFM update, NuGet updates, API migration). The `modernize-dotnet-upgrade-engineer` agent handles the detailed breakdown during execution.
