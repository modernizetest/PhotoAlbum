...
| MCP server fails to start | Verify `dnx Microsoft.GitHubCopilot.AppModernization.Mcp --yes --prerelease --add-source https://api.nuget.org/v3/index.json` runs without errors |
| No scenarios discovered | Ensure you are in a directory with a `.sln`, `.slnx` or `.csproj` file |
