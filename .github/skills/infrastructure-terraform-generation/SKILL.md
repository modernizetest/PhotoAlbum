...
4. **Generate infra-config.md** after successful provisioning following the template in [../infra-config-template.md](../infra-config-template.md). This file is critical for downstream tasks (deployment, integration tests) to discover provisioned resources. Key requirements:
   - Include subscription ID, resource group, and location
   - List all Azure resources with their key properties in the template.
   - DO NOT include any sensitive information such as connection strings or credentials in this file. Instead, only include the names of environment variables where such information is stored, and ensure those environment variables
