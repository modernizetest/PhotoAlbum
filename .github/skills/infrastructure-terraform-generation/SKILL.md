...
4. **Generate infra-config.md** after successful provisioning following the template in [../infra-config-template.md](../infra-config-template.md). This file is critical for downstream tasks (deployment, integration tests) to discover provisioned resources. Key requirements:
   ...
   - Retrieve actual resource values using Terraform outputs or Azure CLI commands
...
- infra-config.md is generated with actual provisioned resource information following the template format
- The JSON block in infra-config.md contains valid, parseable resource metadata
