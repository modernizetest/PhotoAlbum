...
- modernization-work-folder (Mandatory): The folder containing plan.md and tasks.json
...
1. **Read current state**: Read the following files from `${modernization-work-folder}`:
   ...
   - `tasks.json` — the current structured task list (source of truth for execution state)

2. **Detect inconsistencies**: Compare plan.md against the tasks defined in tasks.json. Look for:
...
3. **Update tasks.json**: If inconsistencies are found, rewrite tasks.json to match plan.md:
