...
- **Auth requirements depend on the infra decision table.** A test only needs cloud credentials for the dependencies its TC actually touches as **real** (per Step 2). If every dependency a given TC touches is marked **mock**, the test MUST NOT require `az login` / MI / service-principal credentials at all — wire the mocked SDK clients in the test config and skip the credential pre-flight for that test.
- **Pre-flight auth check** in test setup, applied per real dependency the test touches.
  ...
  - **The pre-flight check is a fail-fast gate, not a silent-skip.** If credentials for a real dependency are missing or insufficient, the test MUST fail loudly (assertion failure / explicit error) so Step 6 picks it up as an **Infra issue** and escalates. Do NOT implement "gracefully abort / mark as skipped / return early" patterns _(e.g. `Assumptions.assumeTrue(credentialsAvailable)`, `Skip.If(...)`, JUnit `@EnabledIfEnvironmentVariable`, early `return` in `@BeforeAll`, try/catch that swallows the auth exception)_ — those let the verification phase complete with zero real assertions executed against the migrated stack.
  - **All-mock tests do not skip on missing credentials.** If a test's dependencies are all mock, it must run unconditionally; a missing `az login` is not a valid reason to skip an all-mock test.

#### Cleanup rules

- **Cleanup via entry points first.** Prefer the application's own delete entry point for cleanup so test credentials need no extra data-plane permissions. Fall back to SDK cleanup only when no delete entry point exists. Run cleanup in `finally` / teardown; ignore "not found". Never use SDK cleanup as a workaround for a broken application delete path — if the application's delete entry point fails, that is a production bug (Step 6), not a cleanup-strategy choice. For mocked dependencies, cleanup is reset of in-memory state — still required so tests stay independent.
...
Run all `*PostMigrationIT` tests. Required: **100% pass**, and the run MUST be a real execution that reached the application. For each TC, the dependencies declared **real** in the infra-decision-table must be hit as real (no silent stubbing) and the dependencies declared **mock** must be served by the configured test doubles (no accidental real call).
...
- **Infra blocker scope is limited to TCs that touch real dependencies.** If the IT run cannot be started or completed for a TC that touches **real** dependencies (no credentials, no network reachability to the real resources, missing role assignment, infra not provisioned, host has no `az login` and no MI), this is an **Infra issue** per the classification below — set the task status to `pending`, hand over to the Infra agent if one exists, otherwise call `ask_user`. The same blocker is **not** a valid reason to skip TCs whose dependencies are all **mock**: those tests must still execute and pass before the task can be `success`.
...
**Task status outcome (mandatory mapping).** Pick exactly one based on the actual runtime result — never on "compile passed" or "unit tests passed". The `real-deps coverage` column refers only to TCs whose decision-table row is **real**:

| Situation | Status |
|---|---|
| Every `*PostMigrationIT` ran AND 100% pass; real-dep TCs hit real resources, mock-dep TCs were served by the configured doubles | `success` |
...
| Real-dep TCs could not be started or completed and the blocker cannot be resolved (Infra agent unavailable or declined, `ask_user` declined to provide credentials / provision infra, infra drift cannot be fixed in this run) | `failed` |
...
The status `success` is permitted only when the Runtime Execution Evidence required by Step 8 can be filled in with real numbers for every TC. If you would have to write "integration tests were not executed because…" for any TC in the summary, the status is **not** `success` — it is `pending` (handoff active on a real-dep TC) or `failed` (handoff exhausted, mock-dep TCs unrun, or no path forward).
...
- **Infra issue** — auth/configuration/network/endpoint problem on the real resource (e.g. AAD user or managed identity not created on the database / storage / Service Bus, missing role assignment, endpoint unreachable, database not created, resource is not configured correctly, or the IT run could not be started for any of these reasons).

  **Action:** set the task status to `pending` per the teams SOP, hand over to the Infra Expert if one exists, otherwise call `ask_user`. Only the Infra Expert has the permissions to provision resources, grant role assignments, and update system-wide configuration consistently; do not attempt these changes from inside the test skill.

  **Hand-back must include:**
  - Which `*PostMigrationIT` could not run.
  - The exact error or missing prerequisite.
  - The resource(s) involved (resource group, account/namespace/server names).

  **Prohibited responses:**
  - Do not patch the test's auth setup beyond what Step 5 prescribes.
  - Do not change the resource-side identity expectation (e.g. datasource username) from inside the test.
  - Do not mark the task `success` while the infra issue is outstanding.
  - Do not loop indefinitely or flip to `success`.

  **Exhausted hand-off:** if the Infra agent is unavailable or declined, or `ask_user` cannot unblock, or the user declines to provision/grant access in this run → close the task as `failed` per the status mapping above.
...
- A per-TC breakdown of **real vs mock dependencies actually used** at runtime, matched against the Step 2 decision table. For real-dep TCs: the authentication mode used (`az login` principal name, MI client ID, or CI SP) and the target resource identifiers (resource group + account/namespace/server) that were actually contacted. For mock-dep TCs: the test-double mechanism used (e.g. `@MockBean`, WireMock stub, in-memory fake) so it is auditable that no accidental real call occurred.

If this section cannot be populated with real execution data for every TC (real-dep TCs with real-resource evidence, mock-dep TCs with mock-wiring evidence), the task is not eligible to be marked `success` — go back to Step 6 and resolve the Infra issue (real-dep TCs) or fix the missing mock execution (mock-dep TCs) first.
