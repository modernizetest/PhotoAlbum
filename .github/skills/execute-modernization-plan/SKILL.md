...
2. Load all tasks from ${modernization-work-folder}/tasks.json and execute them one by one in the order they appear in the `tasks` array in tasks.json (do not reorder tasks):
    ...
    - **Playbook Code Evidence**: If the task has `playbookRules`, after marking the task as success, also write `playbookEvidence` to tasks.json. For each key playbook rule addressed by this task, include 1-3 representative diff lines that demonstrate the rule was implemented. Focus on the most impactful changes (e.g., version number changes, new imports, configuration changes). Each diff line should be formatted as `+ filepath:line  code` or `- filepath:line  code`. Limit to the 2-5 most important rules; skip trivial or repetitive evidence.
