...
2. Load all tasks from ${modernization-work-folder}/.metadata/tasks.json and execute them one by one in the order they appear in the `tasks` array in tasks.json (do not reorder tasks):
    ...
    - Choose the right custom agent to execute the task based on the `type` field of the task in tasks.json, and call the custom agent with the prompt according to the task type and information in tasks.json. The custom agent will return the execution result including whether the task is successful, skipped or failed, and a summary of the execution.
        ...
        6. Custom agent usage to complete setup baseline and integration test tasks:
        For tasks with `"type": "setupBaseline"` in tasks.json, call custom agent general-purpose with prompt:
                ```md
                Call skill create-test-baseline to create the frozen behavior baseline
                Create the frozen test baseline before migration work starts.
                Here is the setup baseline task detail:
                    - TaskId (from `id` field)
                    - Description (from `description` field)
                    - Requirements (from `requirements` field)
                    - migration-scope: {modernization-description}
                    - modernization-work-folder: The folder to save the modernization plan from input
                ```

        For tasks with `"type": "integrationTest"` in tasks.json:
        - If this plan contains a setupBaseline task, call custom agent general-purpose with prompt:
        - Otherwise call custom agent `general-purpose` with prompt:
                ```md
                If this plan contains a setupBaseline task, Call skill verify-test-baseline to verify the frozen behavior baseline and verify the frozen baseline after migration.
