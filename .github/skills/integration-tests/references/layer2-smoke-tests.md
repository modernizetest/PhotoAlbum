...
Layer 2 does **NOT** generate test classes. The runner script (`run-layer2-tests.sh` / `run-layer2-tests.ps1`) **is** the test — it provisions infrastructure, starts the app, runs health checks via `curl` and shell built-ins, and reports pass/fail. No JUnit, xUnit, pytest, or Jest code.
...
Create `docker-compose.smoke.yml` provisioning ALL required dependencies. Every service must have a healthcheck. Include proper ports and volume mounts.

### Step 3: Generate Runner Scripts and Commit Artifacts

Generate `run-layer2-tests.sh` and `run-layer2-tests.ps1` (see Runner Script Template below). Then commit all generated artifacts:
...
This commit initially contains only generated files (`docker-compose.smoke.yml`, runner scripts). No source code changes yet.
...
This includes source code bugs discovered during smoke testing. The commit message says "Add smoke test artifacts" but the commit will contain both test infrastructure AND source code fixes needed for tests to pass. This ensures users who checkout the auth commit (Step 4) will have all fixes via this commit in git history.
...
|-----------|----------|---------------|----------------|
| Spring Boot, `@SpringBootApplication` | Java Web/API | `./mvnw spring-boot:run` | HTTP health probe |
| `Controllers/`, `WebApi` | .NET Web/API | `dotnet run` | HTTP health probe |
| `wwwroot/`, React/Angular/Vue frontend | Frontend | `npm start` | HTTP root path returns HTML |
| `Worker.cs`, `BackgroundService`, `@Scheduled` | Worker/Background | `dotnet run` / `./mvnw spring-boot:run` | Process stays alive for N seconds |
| `main()` with no server | CLI/Batch | `java -jar` / `dotnet run` | Process exits with code 0 |
...
# OR on Windows:
powershell .github/integration-tests/run-layer2-tests.ps1
```
...
- Layer 2 does NOT use JUnit/xUnit/pytest/Jest test classes
- The runner script IS the test - it uses shell commands (curl, grep, etc.)
- Do NOT create any `.java`, `.cs`, `.py`, `.js` test files for Layer 2
...
Generate `run-layer2-tests.sh` and `run-layer2-tests.ps1` in `{modernization-work-folder}`. These are test runners that start infrastructure, build the app, run smoke tests, and report results.
...
Users run:
- Unix: `bash {modernization-work-folder}/run-layer2-tests.sh`
- Windows: `powershell {modernization-work-folder}/run-layer2-tests.ps1`
