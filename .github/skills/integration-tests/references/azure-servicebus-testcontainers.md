...
- 11. Handle ServiceBusAdministrationClient
- 12. Emulator Readiness and Retry
- 13. Emulator fails to start
- 14. Common Gotchas
- You can reference https://learn.microsoft.com/en-us/azure/service-bus-messaging/test-locally-with-service-bus-emulator for more info.
...
If a Service Bus queue/topic/subscription is not created using the Service Bus Admin client, it should be created before the application starts. So every such resource need to be declared in the config.json. So scan the project to see what queues/topics/subscriptions need to be declared. And then define them in config.json.
...
class ServiceBusEmulatorL1Test {
    ...
    static final GenericContainer<?> SERVICE_BUS = new GenericContainer<>(
            ...
            .withCopyFileToContainer(
                    ...
                    "/ServiceBus_Emulator/ConfigFiles/Config.json") // the target file should be uppercase
    ...
    static String getAdminConnectionString() {
        return String.format(
            "Endpoint=sb://%s:%d;SharedAccessKeyName=RootManageSharedAccessKey;"
            + "SharedAccessKey=SAS_KEY_VALUE;UseDevelopmentEmulator=true;",
            SERVICE_BUS.getHost(), SERVICE_BUS.getMappedPort(5300));
    }
...
```
adminConnectionString = "Endpoint=sb://127.0.0.1:5300;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=SAS_KEY_VALUE;UseDevelopmentEmulator=true;";
```

## Handle ServiceBusAdministrationClient
SDKs in languages other than .NET do not honor non‑TLS connections or custom ports in the connection string. As a result, they do not natively support the Emulator’s management APIs. Limited workarounds are available to configure these SDKs to communicate with the Emulator over non‑TLS and custom ports. Refer to the code samples for details.
...
1. Use a test configuration that use a customized admin client, using the admin connection string of the emulator.
...
Inject another admin bean only for integration testing.
