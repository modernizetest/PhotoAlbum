...
- Principles
  + 1.Core Principles (naming convention, no source code modification)
  ...
  + 13.DO write specific, descriptive assertions using AssertJ when available
...
Migrated applications typically use **Managed Identity** (`DefaultAzureCredential`) which only works on Azure infrastructure. For local integration testing applications must connect to local emulators.
...
**Examples:** Plain Java CLI apps without config frameworks, any application that hardcodes connection strings directly in source code
...
Always instantiate and invoke the project's own classes/methods. Never call third-party library APIs directly in the test as a substitute for testing the application. If the application has a service class with methods like `handleRequest()`, `processData()`, or `createBucket()`, your tests MUST call those methods. 
   ...
   **Correct pattern — invoke the application's own entry points:**
   ...
   ```
   If the application class cannot be instantiated directly (e.g., it reads from `System.getenv()`), apply principle #3 to make it testable with a minimal source change (e.g., add a constructor that accepts the client). **Never bypass the application class by re-implementing its logic with direct SDK calls.**

   **Self-check before writing each test:** Ask "Does this test call at least one method on the application's own class?" If the answer is no, the test is wrong — rewrite it to go through the application code.
...
If production code cannot be invoked from a test, introduce the smallest possible change (e.g., adding a method, widening visibility, extracting a parameter) to enable it. Never alter existing behavior, signatures, or control flow. Never work around untestable code by re-implementing its logic in the test.
   ...
   > If the application uses Azure Blob Storage and constructs a `BlobServiceClient` internally, you MUST handle the Azurite URL format difference. See the [Azurite URL Format Gotcha](./azure-storage-testcontainers.md#2-azurite-url-format-gotcha) for details on injecting `BlobServiceClient` and container name as constructor parameters.

### 4.Wire and test the full execution path

Tests must exercise the end-to-end flow from input through business logic to output, not individual layers (controller layer or database layer) in isolation. Verify the multi-step integration delivers the business outcome, not just that SDK methods execute.
...
### 7.Test error handling at the application level with precise assertions — this is MANDATORY, not optional.
At least 30% of test methods must cover error, edge-case, or negative scenarios. Happy-path-only test suites are incomplete. For every class under test, identify its error-handling paths (catch blocks, null checks, validation logic, fallbacks) and write tests that exercise them through the application's own methods.

   **Required error test categories — you MUST write at least one test for EACH category that applies:**
   - **Exception wrapping:** If the application catches SDK exceptions and wraps them, test that the wrapper exception type, message, and cause are correct. Use `assertThrows()` and inspect `getCause()`.
   - **Resource-not-found handling:** Test what happens when the application reads a non-existent resource (blob, database row, cache key). Assert on the specific return value (null, empty Optional, custom error) — not just that no exception is thrown.
   - **Invalid input handling:** Test null, empty string, boundary-length, and malformed inputs through the application's public methods. Assert on the specific exception type or error response.
   - **HTTP status codes / error codes:** When the application produces or handles status codes (404, 409, 400), assert on the specific code, not just that an error occurred.
   - **Error message content:** When the application produces error messages (e.g., in RuntimeException constructors), assert the message contains expected content.
   - **Graceful degradation:** If the application depends on a networked external service (Redis, Azure Blob, database), consider testing behavior when the service is unavailable or returns unexpected errors.
   - **Serialization round-trips:** If the application serializes/deserializes data (e.g., Java objects to JSON for Redis, or different SDK types), test that data survives the round-trip including edge cases (null fields, special characters, Unicode, timestamps). Also test what happens with corrupted/invalid data in the store.
...
### 11.DO NOT use any test ordering mechanism — this is an AUTOMATIC QUALITY DEDUCTION
...
This includes JUnit 5 `@Order` / `@TestMethodOrder`, TestNG `dependsOnMethods` / `priority`, and any other framework-specific ordering. Tests MUST be independent and run in any order. Each test must create its own data and clean up after itself. Test ordering indicates shared mutable state — a critical quality issue. Evaluators specifically check for `@Order` and `@TestMethodOrder` annotations and will deduct points for their presence.
   ...
   **Correct pattern:**
   ...
   ```
### 12.DO NOT use reflection to access or modify private fields, and DO NOT widen field visibility (private->protected/public) in production code for test access.
...
### 13.DO write specific, descriptive assertions using AssertJ when available
If the project already has AssertJ on its classpath (common with Spring Boot via `spring-boot-starter-test`), use it exclusively — do not mix with JUnit `assertTrue`/`assertEquals`. AssertJ provides far better failure diagnostics:
   ...
   If the project does NOT already have AssertJ, use JUnit 5 assertions with descriptive failure messages. Regardless of library, always assert on specific values — not just `assertNotNull(result)` or `assertTrue(success)`. Include `assertInstanceOf()` (JUnit 5.8+) instead of `assertTrue(x instanceof Y)` for better error messages.
...
**You MUST follow this order.** Skipping ahead to writing tests before configuring the build is the #1 cause of failing integration test suites.

1. **Analyze** -> Scan app dependencies, identify migrated classes and their catch blocks
2. **Verify build** -> Create a smoke `*L1Test.java` and confirm `mvn verify` discovers it (section 2b)
3. **Plan tests** -> List all test methods (happy path + error) and verify 30%+ are error tests
4. **Write tests** -> Implement tests that invoke application classes (not SDK directly)
5. **Run & Fix** -> Execute tests, analyze failures, iterate
6. **Verify discovery** -> Confirm `mvn verify` count matches expectations
7. **Generate scripts** -> Create runner scripts