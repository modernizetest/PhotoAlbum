...
- **push-git-url** (Optional): GitHub repository URL to push the generated playbook to. When set, output-path is NOT provided — the pre-sync script determines the output path. Do NOT write files to any other location.
...
If ${push-git-url} is set, run the pre-sync script to clone/update the repo and set up the branch:

```bash
# For Unix/Linux/Mac
./scripts/playbook-pre-sync.sh ${push-git-url}
```

```powershell
# For Windows (PowerShell)
.\scripts\playbook-pre-sync.ps1 -RepoUrl ${push-git-url}
```

The script outputs the playbook output path on its **last line of stdout**. Use this as ${output-path} for all subsequent steps.

If the script exits with a non-zero code, stop and report the error message from stderr to the user.
...
### Step 4: Push to Remote Repository (only if push-git-url is provided)

**This step is MANDATORY when ${push-git-url} is set. Do NOT skip it.**

Run the post-sync script to commit, push, and create/update a PR:

```bash
# For Unix/Linux/Mac
./scripts/playbook-post-sync.sh ${push-git-url} "<summary>"
```

```powershell
# For Windows (PowerShell)
.\scripts\playbook-post-sync.ps1 -RepoUrl ${push-git-url} -Summary "<summary>"
```

For the `<summary>` argument, write a brief one-paragraph summary of what was added/changed in this sync (e.g. "Added 3 target frameworks, 4 prohibited technologies, and 3 security standards from Phase 2 migration policy.").

Read the **last line of stdout** from the script:
- If it contains a URL → this is the PR URL. Report it to the user: "Pull request: <URL>"
- If it is "NO_CHANGES" → report "No changes detected, nothing to push."
...
- If push-git-url was set: the PR URL from Step 4
