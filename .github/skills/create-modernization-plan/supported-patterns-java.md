...
- **migration-AWS-secrets-manager-to-azure-key-vault**: Migrate AWS Secrets Manager to Azure Key Vault. Use when migrating secret storage, retrieval, and Spring Cloud secret property sources from AWS to Azure.
...
- **migration-javax.email-send-to-azure-communication-service-email**: Migrate JavaMail to Azure Communication Services Email. Use when replacing javax.mail email sending with Azure Communication Services.
...
- **migration-local-files-to-mounted-azure-storage**: Migrate local file paths to Azure mounted storage. Use when replacing hardcoded or platform-specific file paths with Azure Storage mount paths.
...
- **migration-plaintext-credential-to-azure-keyvault**: Migrate plaintext credentials to Azure Key Vault. Use when moving hardcoded passwords, API keys, and tokens from source code and config files to Azure Key Vault.
- **migration-s3-to-azure-blob-storage**: Migrate AWS S3 to Azure Blob Storage. Use when replacing S3 client operations, bucket management, and object storage with Azure Blob Storage SDK equivalents.
...
- **migration-sqs-to-servicebus**: Migrate AWS SQS to Azure Service Bus. Use when replacing SQS message sending and receiving with Service Bus sender and receiver clients.
