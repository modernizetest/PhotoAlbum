...
3. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
    ...
    **Success Criteria Guidelines**: When generating successCriteria for transform and upgrade tasks:
    - **CRITICAL**: Do NOT include `generateNewIntegrationTests` or `passIntegrationTests` properties
    - Only include: `passBuild`, `generateNewUnitTests`, and `passUnitTests`
    - Integration testing is handled by the separate integrationTest task, not by transform/upgrade task success criteria
