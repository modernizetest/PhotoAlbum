...
3. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
    ...
    **Playbook Rules Mapping**: When playbook policy files are provided as attachments:
    - Write the playbook metadata to `metadata` in tasks.json: `playbookSource`, `playbookFiles`, `playbookContentHash` (these values are provided in the additional instructions)
    - For each task, annotate `playbookRules` with a list of playbook rules that the task addresses. Each entry has `file` (the playbook filename) and `rule` (a brief summary of the specific rule from that file). A task may cover multiple rules, and a rule may be covered by multiple tasks. Only include rules that the task directly addresses.
