...
This skill is used to create a modernization plan to migrate a given project to Azure.
...
- `github-issue-link` (Optional): A GitHub issue to track modernization status, to be filled into the plan template.
- `assessment-report` (Optional): An assessment report for the project being modernized. It provides project data for planning.
- `plan-name` (Optional): The plan name to be filled into the plan template.
...
1. Double-check the issues
   **IMPORTANT**:
   - If you are given an assessment-report, you need to double check if the issue really exists in the current project. If not, please ignore this issue when you generate the plan

2. **Load context**: Retrieve information for plan, you can read
    1. Analyze the supported patterns to find the right tasks for the issues.
    2. Analyze modernization requirements from user input.
...
3. **Generate plan and tasks**: Generate plan.md and tasks.json using the appropriate templates:
    ...
    **Plan Generation**:
    ...
    2. Follow the rules defined in the template to fill in sections with relevant information based on analysis of user input and mentioned files.
    3. Save the plan in `${modernization-work-folder}` with the filename `plan.md`. If a plan already exists, overwrite it.
    ...
    5. Save the tasks in `${modernization-work-folder}` with the filename `tasks.json`. If `tasks.json` already exists, overwrite it.
