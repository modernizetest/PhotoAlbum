...
- **migration-azure-app-configuration**: Migrate application configuration to Azure App Configuration. Use when migrating local configuration files (appsettings.json, Web.config, App.config), ConfigurationManager.AppSettings, hardcoded config values, environment variables, Windows INI files, or feature flags to Azure App Configuration with Key Vault secret references for centralized configuration management.
