...
- **Always add the azure-identity-extensions dependency.** This library provides the PostgreSQL authentication plugin that enables passwordless managed identity connections to Azure Database for PostgreSQL. Without it, the authentication plugin class referenced in the JDBC URL will not be found at runtime, causing `ClassNotFoundException`.
- **Include all required managed identity parameters in the JDBC URL.** Omitting any of these parameters (especially `authenticationPluginClassName`, `azure.managedIdentityEnabled`, or `azure.clientId`) will cause the connection to fall back to password-based authentication, which will fail in a managed identity environment.
- **Comment out username/password properties — do not convert them to environment variables.** Managed identity authentication does not use username/password. Leaving these properties active (even as env vars) misleads operators into thinking they need to supply credentials. Commenting them out with a note makes the migration intent clear and avoids confusion.
- **Set allocationSize=1 on all @SequenceGenerator annotations.** PostgreSQL sequences increment by 1 by default, but JPA's default `allocationSize` is 50.
