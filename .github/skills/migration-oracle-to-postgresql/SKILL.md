...
- Enable passwordless connection. Use managed identity by default. Add comments to show how to authenticate by service principal.
    > **Note:** The passwordless/managed identity authentication approach below uses `AzurePostgresqlAuthenticationPlugin`, which is JDBC-only and does **not** support R2DBC.
