...
- Locate the `coding_notes.md` file:
...
- If `coding_notes.md` is found, read its entire content before listing files that need to be migrated.
...
- Review the KB checklist items below. **Priority rule**: If any checklist item conflicts with guidance in `coding_notes.md`, follow the `coding_notes.md` instructions instead.
- Apply each relevant modification to the file.
- For each change, add a comment to the migrated code that includes the checklist item information. Comment style must match the target file type. Example comment in Java code:
    ```java
...
- KB check item 1: Enable passwordless connection. Use managed identity by default. Add comments to show how to authenticate by service principal.
    ...
    - In property file, add example PostgreSQL JDBC URL to show how to authenticate by service principal.
        ...
        - Do not add default values for environment variables.
            ...
            ```
- KB check item 2: Use lowercase for identifiers (like table and column names) and data types (like varchar). Use uppercase for SQL keywords (like `SELECT`, `FROM`, `WHERE`). This includes SQL statements and JPA annotations like `@Table`, `@Column`, `@NamedNativeQuery`, and `@Query`.
    ```diff
    - String sql = """
