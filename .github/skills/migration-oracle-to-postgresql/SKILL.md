...
1. Locate the `coding_notes.md` file:
    ...
    - If no `coding_notes.md` file is found, proceed to step 3 and follow only the checklist below.
...
3. Review the checklist items below. **Priority rule**: If any checklist item conflicts with guidance in `coding_notes.md`, follow the `coding_notes.md` instructions instead.
...
5. For each change, add a comment to the migrated code that includes the checklist item information. Comment style must match the target file type. Example comment in Java code:
    ```java
    // Migrated from Oracle to PostgreSQL according to checklist item 1: Enable passwordless connection.
...
- Checklist item 0: Don't modify any content that is not related to Oracle to PostgreSQL migration.
- Checklist item 1: Enable passwordless connection. Use managed identity by default. Add comments to show how to authenticate by service principal.
    ...
    - In property file, add example PostgreSQL JDBC URL to show how to authenticate by service principal.
        ...
        - Do not add default values for environment variables.
            ...
            ```
- Checklist item 2: Use lowercase for identifiers (like table and column names) and data types (like varchar). Use uppercase for SQL keywords (like `SELECT`, `FROM`, `WHERE`). This includes SQL statements and JPA annotations like `@Table`, `@Column`, `@NamedNativeQuery`, and `@Query`.
    ```diff
    - String sql = """
    ...
    + }
    ```
- Checklist item 9999: Migrate all other Oracle-specific content to PostgreSQL. Verify each change is functionally equivalent and compatible.
