...
1. Locate the `coding_notes.md` file:
   ...
   - If no `coding_notes.md` file is found, proceed to step 3 and follow only the KB checklist below.
...
3. Review the KB checklist items below. **Priority rule**: If any checklist item conflicts with guidance in `coding_notes.md`, follow the `coding_notes.md` instructions instead.
4. Apply each relevant modification to the file.
5. For each change, add a comment to the migrated code that includes the checklist item information. Comment style must match the target file type. Example comment in Java code:
```java
// Migrated from Oracle to PostgreSQL according to KB check item 1: Enable passwordless connection.
...
KB check item 0: Don't modify any content that is not related to Oracle to PostgreSQL migration.

KB check item 1: Enable passwordless connection. Use managed identity by default. Add comments to show how to authenticate by service principal.
**IMPORTANT - JDBC only**: The following steps only work for JDBC connections. If the project uses R2DBC (check for `r2dbc:` URLs or `r2dbc-postgresql` dependencies), ignore this entire check item.

1. In Java code, comment out all "username" and "password" related content which are ONLY corresponding to the PostgreSQL JDBC URL.
```diff
...
2. In build config file (like pom.xml), add this dependency:
```diff
+ <dependency>
...
+ </dependency>
```
3. In property file, update the PostgreSQL JDBC URL to support authentication by managed identity.
   Follow these steps:
   3.1. Add these parameters to the PostgreSQL JDBC URL:
   ...
   3.2. Use environment variable for database host/port/database name if the original value is not Azure PostgreSQL.
   3.3. Add comments about environment variables in the PostgreSQL JDBC URL.
   3.4. Comment out all "username" and "password" related content which are ONLY corresponding to the PostgreSQL JDBC URL.
   3.5. Do not add default values for environment variables.
```diff
...
+ # password: testpass
```
4. In property file, add example PostgreSQL JDBC URL to show how to authenticate by service principal.
   4.1. These parameters are required:
   ...
   4.2. Do not add default values for environment variables.
```diff
...
KB check item 2: Use lowercase for identifiers (like table and column names) and data types (like varchar). Use uppercase for SQL keywords (like `SELECT`, `FROM`, `WHERE`). Including SQL statements and JPA annotations like `@Table` and `@Column`, `@NamedNativeQuery`, `@Query`).
```diff
- String sql = """
...
KB check item 9999: Migrate all other Oracle-specific content to PostgreSQL. Verify each change is functionally equivalent and compatible.
