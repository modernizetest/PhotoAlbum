...
## Category 1: Managed Identity Dependencies

### Always add the azure-identity-extensions dependency

Add `com.azure:azure-identity-extensions:1.2.2` to `pom.xml`.

**Why:** This library provides the PostgreSQL authentication plugin that enables passwordless managed identity connections to Azure Database for PostgreSQL. Without it, the authentication plugin class referenced in the JDBC URL will not be found at runtime, causing `ClassNotFoundException`.
...
## Category 2: JDBC URL Configuration

### Include all required managed identity parameters in the JDBC URL

The PostgreSQL JDBC URL must contain the full set of managed identity authentication parameters:

```
jdbc:postgresql://${PGHOST}:${PGPORT}/${PGDATABASE}?user=${MANAGED_IDENTITY_NAME}&sslmode=require&authenticationPluginClassName=com.azure.identity.extensions.jdbc.postgresql.AzurePostgresqlAuthenticationPlugin&azure.managedIdentityEnabled=true&azure.clientId=${MANAGED_IDENTITY_CLIENT_ID}
```

**Why:** Omitting any of these parameters (especially `authenticationPluginClassName`, `azure.managedIdentityEnabled`, or `azure.clientId`) will cause the connection to fall back to password-based authentication, which will fail in a managed identity environment.

### Use environment variables for host, port, and database — without default values

Use `${PGHOST}`, `${PGPORT}`, `${PGDATABASE}` — never `${PGHOST:localhost}` or `${PGPORT:5432}`.

**Why:** Default values create a false sense of working configuration. In production, a missing environment variable should fail fast rather than silently connect to `localhost`, which would either fail or connect to the wrong database.

### Add comments explaining environment variables and sovereign cloud parameters

Include comments that document each environment variable's purpose and list the sovereign cloud parameters (`azure.scopes`, `azure.authorityHost`) needed for non-public Azure clouds.

**Why:** Operators deploying to Azure Government or Azure China need to know which additional JDBC URL parameters to set. Without these comments, sovereign cloud deployments will fail with authentication errors because the default authority host and scope target the public cloud.

### Add a commented-out example URL for service principal authentication

Include a commented example showing the service principal variant with `user=${SERVICE_PRINCIPAL_NAME}`, `azure.clientId`, `azure.clientSecret`, and `azure.tenantId` parameters.

**Why:** Some deployments use service principals instead of managed identities. Providing a ready-made template prevents operators from guessing the correct parameter names and reduces misconfiguration risk.

## Category 3: Credential Removal

### Comment out username/password properties — do not convert them to environment variables

When migrating to managed identity, comment out (with an explanatory note) all `username`/`password` properties and programmatic setters like `dataSource.setUsername(...)` / `dataSource.setPassword(...)`. Do not replace them with `${POSTGRES_USERNAME}` or similar env-var references.

**Why:** Managed identity authentication does not use username/password. Leaving these properties active (even as env vars) misleads operators into thinking they need to supply credentials. Commenting them out with a note makes the migration intent clear and avoids confusion.

## Category 4: JPA / Hibernate Compatibility

### Set allocationSize=1 on all @SequenceGenerator annotations

When migrating `@SequenceGenerator` from Oracle to PostgreSQL, always explicitly set `allocationSize = 1`.

```java
@SequenceGenerator(name = "my_gen", sequenceName = "my_seq", allocationSize = 1)
```

**Why:** PostgreSQL sequences increment by 1 by default, but JPA's default `allocationSize` is 50. This mismatch causes JPA to reserve IDs 1–50 while the database sequence only issued 1. The next insert from a different session gets ID 2 from the sequence, colliding with JPA's pre-allocated range — resulting in primary key constraint violations.
