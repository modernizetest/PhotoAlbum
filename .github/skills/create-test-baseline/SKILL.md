...
Create a test baseline for the project to be modernized. The baseline captures API/UX-level behavior that must be preserved across migration. It produces a `test-cases.md` describing all test scenarios, then generates `*BaselineIT` test classes from those scenarios.
...
- Baseline tests describe behavior at the **API/UX boundary only** — HTTP endpoints, CLI commands, or public API surfaces.
...
│   └── *BaselineIT            # Behavior tests at HTTP / API boundary
└── testdata/
...
Analyze the application's API/UX surface affected by the migration scope. Produce `test-cases.md` under `<module>/test/` using [test-cases-template.md](test-cases-template.md) as the template.

**Requirements:**
- Identify all API endpoints, CLI commands, or public operations affected by the migration scope.
- For each operation, define test cases covering **four buckets**: happy path, boundary values (empty, max, page boundary), special inputs (unicode, reserved characters, missing resource), and failure mapping (simulated backend error → application response).
- Use **2–5 representative records per entity** (table, queue, container, etc.).
- Each test case must describe **only API/UX-level inputs and expected outputs** — no service-specific SDK types, no backend implementation details. These are behaviors visible to the end user or API consumer that must remain unchanged after migration.
...
- Tests must exercise the application at the **HTTP / API boundary** — never by importing service-specific SDK classes.
