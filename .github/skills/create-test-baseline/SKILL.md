...
└── test-cases/                 # FROZEN — entire folder is the baseline spec bundle
    ...
    ├── infra-decision-table.md # FROZEN — mock/real decision per external dependency (Step 2)
...
Scan the production source for everything that constitutes an external boundary or orchestration entry point. Record each one with file path and symbol so it can be cross-checked in Step 4.
...
- Published library / SDK methods that external code calls
...
Produce `<test-source-root>/test-cases/test-cases.md` using [test-cases-template.md](test-cases-template.md). For each entry point, cover the **four coverage buckets**:
...
### Step 4: Build Infra Decision Table (mock vs real)

With the full set of test cases and their referenced dependencies now visible, decide which external dependencies the post-migration tests will exercise as **real** resources and which will be **mocked**. This decision is recorded once here and is reused as-is by `verify-test-baseline` — verification does not re-decide.

**Inputs:**
- The exhaustive list of external dependencies actually touched by the test cases written in Step 2 (cross-checked against the entry-point inventory from Step 1).
...
**Rules:**
- Dependency present in `infra/` (provisioned endpoint + credentials) → **real**. Mocking it is a bug.
- Dependency not present in `infra/` → **mock** at the SDK / HTTP boundary, seeded from `testdata/`.
- If `infra/` does not exist at all, mark every external dependency as **mock** and record `infra-missing` as the reason.
...
This file is part of the frozen baseline bundle (see Step 6) and is consumed as-is by `verify-test-baseline`. Do not proceed to Step 5 until the table is saved.

### Step 5: Validate the Spec (Required Field Checklist)
...
| 4 | `Entry Point` | Exact identifier from production code: HTTP method+path, fully qualified method, CLI command, queue/topic name. No vague references. |
...
**Entry-point coverage check**: every orchestration entry point cataloged in Step 1 appears as the `Entry Point` of at least one `happy-path` test case AND at least one `failure` test case.

Any validation failure → fix the spec. Do not freeze with defects.

### Step 6: Freeze and Output
...
2. Create `${modernization-work-folder}/${taskid}/baseline-summary.md` summarizing: entry-point inventory, test case count by category, list of `test-cases/testdata/` files, the Step 2 infra decision table (mock vs real per dependency), and confirmation that the Step 5 checklist passed for every case.
