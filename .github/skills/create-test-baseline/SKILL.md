...
### Step 3a: Composite Scenario Coverage for HTTP modules

After Step 3's per-endpoint coverage, run this sub-routine for each in-scope module that exposes **HTTP/REST** entry points (Spring MVC incl. WebFlux, JAX-RS, Micronaut HTTP, or the Servlet API). It adds **cross-endpoint, stateful** test cases (create → read → mutate → delete flows that span several controllers) which a per-endpoint pass tends to miss. Modules with no HTTP entry points skip it and rely on Step 3 alone.

The sub-routine is grounded in a **mandatory** static-analysis run so every scenario step references a real endpoint:

1. **Layer 0 — static endpoint model (forced).** First run the **preflight environment check** in [scenario-coverage/endpoint-model.md](scenario-coverage/endpoint-model.md): verify Python 3, a JDK, IBM CLDK, and (on Windows) WSL, and install the missing pieces (`pip install cldk` is the usual one). Then run the bundled `endpoint-model-sa.py build <module> endpoint-model.json`, which executes IBM CLDK `codeanalyzer` and emits the framework-neutral endpoint model (the 8-tuple), DTO schemas, auth model, and the deterministic **resource** dependency edges. If a prerequisite cannot be installed and the tool cannot run, Step 3a **fails closed** for that module — produce no composite cases and keep the Step 3 baseline. See [scenario-coverage/endpoint-model.md](scenario-coverage/endpoint-model.md).
2. **Layer 1 — Object Dependency Graph.** Enrich the model with **producer-consumer** and **database** edges by reading handler bodies (following [scenario-coverage/model-enrich.txt](scenario-coverage/model-enrich.txt)), then re-validate it with `endpoint-model-sa.py validate`. Same file.
3. **Layer 2 — scenario authoring.** Walk the validated graph to compose scenarios and write each one **directly** as a single composite `test-cases.md` case (one `TC-<MODULE>-NNN`, multiple HTTP calls inside the `Trigger`; the focal/asserted endpoint is the case's `Entry Point`). See [scenario-coverage/scenario-authoring.md](scenario-coverage/scenario-authoring.md).

Integration with the rest of the workflow:

- Composite cases use the same [test-cases-template.md](test-cases-template.md), the same four `Category` values, and the same `TC-<MODULE>-NNN` ids as Step 3 — they are additional cases in the module's existing `test-cases.md`, not a separate artifact.
- The endpoint model + ODG is **scratch** (written to `${modernization-work-folder}/${taskid}/scenario-coverage/endpoint-model.json`); it is **not** part of the frozen `test-cases/` bundle. No `endpoint-requests.json` or `scenarios.json` is produced.
- Step 5: feed each endpoint's `databaseOperations` and the `authModel` into the infra decision-table dependency rows.
- Step 6: add the **scenario-grounding gate** — every `Trigger`/`Preconditions` step must match a real endpoint by method + normalized path (path-variable segments as `{}`); drop or fix any unmatched step before freezing. Reconcile strict-`4xx` validation-failure claims against the DTO schema (only valid for fields that carry a binding-layer constraint).

See [scenario-coverage/README.md](scenario-coverage/README.md) for the activation gate, artifact flow, and the forced static-analysis prerequisites.