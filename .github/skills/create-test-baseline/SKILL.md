...
### Step 5: Build Infra Decision Table (mock vs real)

With the full set of test cases and their referenced dependencies now visible, decide which external dependencies the post-migration tests will exercise as **real** resources or **mock** at the SDK / HTTP boundary. This decision is recorded once here and is reused as-is by `verify-test-baseline` — verification does not re-decide.
...
- The mock/real decisions already made by existing tests (from Step 2). Prefer consistency with existing tests unless there is a clear reason to diverge.
